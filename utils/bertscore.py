...
class BERTScore():
    ...
    def __init__(self,model_path,
        ...
        if 'bert' in model_path.lower():
            self.model = BertModel.from_pretrained(model_path,device_map=device)
            assert (
    ...
    def sentence_bert_score(self,a,b):
        '''
        @param a: 参考句子
        @param b: 生成句子
        '''
...
if __name__ == '__main__':
    a = 'Hypertension, commonly referred to as high blood pressure, is a prevalent chronic medical condition. It is characterized by elevated blood pressure levels in the arteries. When blood pressure remains consistently above the normal range over a period of time, it can lead to various health complications. For instance, it may cause damage to blood vessels, increasing the risk of cardiovascular diseases such as heart attacks and strokes. Additionally, hypertension can have adverse effects on other organs like the kidneys. Regular monitoring of blood pressure is crucial for early detection and management of this condition. Lifestyle modifications, including a balanced diet, regular physical activity, and stress management, play a significant role in controlling hypertension. In some cases, medications may also be prescribed by healthcare professionals to help lower blood pressure and reduce the associated risks.'
    b = 'High blood pressure, also known as hypertension, is a widespread chronic health issue. It involves persistently high blood pressure in the arteries. When blood pressure stays above the normal threshold for an extended duration, it can result in multiple health problems. It can lead to damage of the blood vessels, which in turn raises the likelihood of cardiovascular conditions such as myocardial infarctions and cerebral strokes. Moreover, hypertension can negatively impact other organs, including the kidneys. Consistent blood pressure checks are essential for timely identification and treatment of this condition. Adopting healthy lifestyle changes, like maintaining a nutritious diet, engaging in regular exercise, and managing stress, are vital for hypertension control. Sometimes, healthcare providers may prescribe medications to help bring down blood pressure and mitigate the potential risks.'
    s = BERTScore('models/GTE/biobert-v1.1',num_layers=8,device='cuda:1')
    c = '\n'
    d = '[]'
    k = s.sentence_bert_score(d,c)
    print(k)

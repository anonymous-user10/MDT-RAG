...
NumberedRows AS (
    ...
    FROM
        admissions_subset a
    JOIN
        mimiciv_hosp.labevents l ON a.subject_id = l.subject_id
)
SELECT
    nr.subject_id as subject_id,
    nr.itemid as itemid,
    nr.value as labevent_value,
    nr.valuenum as valuenum,
    nr.valueuom as valueuom,
    nr.ref_range_lower as ref_range_lower,
    nr.ref_range_upper as ref_range_upper,
    nr.comments as labevent_comments,
    d_labitems.label AS labevent_label,
    d_labitems.fluid AS labevent_fluid,
    d_labitems.category AS labevent_category
FROM
    NumberedRows nr
JOIN mimiciv_hosp.d_labitems d_labitems ON nr.itemid = d_labitems.itemid
WHERE
    rn <= 10; -- T是你设定的阈值

...
NumberedRows AS (
    SELECT
        d_icd_procedures.long_title as d_icd_procedures_long_title,
        l.* ,
        ROW_NUMBER() OVER (PARTITION BY a.hadm_id ORDER BY l.hadm_id) AS rn
    FROM
        admissions_subset a
    JOIN
        mimiciv_hosp.procedures_icd l ON a.hadm_id = l.hadm_id
    JOIN mimiciv_hosp.d_icd_procedures d_icd_procedures ON l.icd_code = d_icd_procedures.icd_code AND l.icd_version = d_icd_procedures.icd_version
)
SELECT
    nr.subject_id as subject_id,
    nr.hadm_id as hadm_id,

    nr.d_icd_procedures_long_title as d_icd_procedures_long_title
